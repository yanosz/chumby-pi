...
| Brightness, night mode | **delivered** | fork FR16 shipped 2026-07-13 (sliders onto a kernel backlight, or a `brightness_ctl` executable); working on hardware since 2026-08-24 on the 5" Waveshare DSI (C), whose kernel backlight `10-0045` the deb's udev rule hands to the `video` group. On a panel without a backlight — the ILI9486 TFT, the HDMI (E) — the Settings button stays honestly disabled |
...
| Item | Blocked on |
|------|-----------|
| **Brightness + night mode** | ~~Hardware only~~ — **resolved 2026-08-24** (claude/issues.md #5). The 5" Waveshare DSI LCD (C) exposes a real kernel backlight (`10-0045`, max 255); the shipped `90-chumby-backlight.rules` makes it `root:video 0664` on a plain install, so the player writes it as `pi` with no sudo, and Jan confirmed the panel's sliders dim the screen. The PWM solder-pad mod on the ILI9486 clone is moot — that panel simply has no backlight to drive, which the `settings-brightness` rule reflects by staying disabled. |
